//Decoding Seed Studio T1000 payload
...
export const handler = async (event) => {
    try {
        ...
        // resolved_data.messages contains an array of array! 
        for (let messages of resolved_data.messages) {
            
            console.info("Measurements: " + JSON.stringify(messages, null, 2) )
            
            for (let message of messages) {
                
                if (message["measurementId"] === "4197") { //Longitude - simple value
                    response.PositionType= "GPS"
                    response.Position.Longitude = message["measurementValue"]
                    containPosition = true
                }
                if (message["measurementId"] === "4198") { //Latitude - simple value
                    response.Position.Latitude = message["measurementValue"]
                }
                if (message["measurementId"] === "3000") { // Battery - simple value
                    console.info("Battery: " + message["measurementValue"] )
                    response.Battery = message["measurementValue"]
                    response.Timestamp = message["timestamp"]
                }
                if (message["measurementId"] === "4200") { // Event Status - array of measurement Values
                    response.Events = message["measurementValue"]
                }



                /* CASE 11 --> no position lock 
                {
                    "measurementId": "3576",
                    "timestamp": 3077650756000,
                    "type": "Positioning Status",
                    "measurementValue": {
                        "id": 1,
                ...
                */
                ...
                if (message["measurementId"] === "3576") {
                    if ( ( message["measurementValue"]["id"] === 1) ||
                         ( message["measurementValue"]["id"] === 12) ||
                         ( message["measurementValue"]["id"] === 13) ||
                         ( message["measurementValue"]["id"] === 14) )
                     {
                         //issue in obtaining the GPS position
                         response.ErrorMessage = message["measurementValue"]["statusName"]
                     }
                }
            }
        }

        if ( containPosition === true ) 
        {
            response.PositionType = "GPS"
            
            console.info("Decoded Message contain position.")
        ...
        else
        {
            ...
            console.info("Decoded Message doesn't contain position!")   
            console.info(response)
